...
- **Level cards i18n fixed (2026-03-30):** Added `data-i18n="card-title-1"` through `card-title-5` attributes to `.card-title` elements in `index.html`. Added corresponding translation entries in `js/i18n.js` (NL: "Geheim dagboek", "De onzichtbare kluis", etc. / EN: "Secret Diary", "The Invisible Vault", etc.). Modified `game.js` line 90 to use `t("card-title-" + level)` for success messages, ensuring level names match selected language throughout the site. Pattern: any hardcoded UI text should get a `data-i18n` attribute and matching translation key.
